...
// defulte initializer
- (instancetype)init{
    return [self initWithchairnum:0 isfurnitreleather:YES length:0 width:0 color:[UIColor redColor] manufactureCompany:@" " manufacture:[NSDate date] model:@" " plateNumber:@" "  bodySerialNumber:@" " engine:[HAEngine new] gearType:Normal];
}

//initializer for parent method
- (instancetype)initWithLength:(NSInteger)length width:(NSInteger )width color:(UIColor *)color manufactureCompany: (NSString *)manufactureCompany manufacture:(NSDate *)manufacture model:(NSString *)model plateNumber:(NSString *)plateNumber bodySerialNumber:(NSString *)bodySerialNumber engine:(HAEngine*) engine{
