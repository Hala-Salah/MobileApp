...
// defulte initializer
- (instancetype)init{
    return[self initWithFreeWight:0.0 fullWight:0.0 length:0 width:0 color:[UIColor redColor] manufactureCompany:@" " manufacture:[NSDate date] model:@" " plateNumber:@" " bodySerialNumber:@" " engine:[HAEngine new]gearType:Normal];
}

//initializer for parent method
- (instancetype)initWithLength:(NSInteger)length width:(NSInteger )width color:(UIColor *)color manufacturCompany: (NSString *)manufactureCompany manufacture:(NSDate *)manufacture model:(NSString *)model plateNumber:(NSString *)plateNumber bodySerialNumber:(NSString *)bodySerialNumber engine:(HAEngine*) engine{
    return[self initWithFreeWight:0.0 fullWight:0.0 length:0 width:0 color:[UIColor redColor] manufactureCompany:@" " manufacture:[NSDate date] model:@" " plateNumber:@" " bodySerialNumber:@" " engine:[HAEngine new] gearType:Normal];
}
@end
